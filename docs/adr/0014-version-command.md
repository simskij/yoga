...
The version string is embedded at build time via `-ldflags "-X main.version=<tag>"`. For untagged builds (local `go build`, `go install` from HEAD) the default value `dev` is used.
