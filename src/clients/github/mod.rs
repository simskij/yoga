...
pub async fn list_issue_counts(_cfg: &Config) -> Result<(), octocrab::Error> {
    ...
    if token.is_empty() {
        println!(
            "Warning: No GITHUB_TOKEN set. Will run anonymously, which might incur rate limits."
        );
    } else {
...
async fn get_repo_counts(crab: &octocrab::Octocrab, repo: &String, kind: &str) -> u64 {
    let res: Result<serde_json::Value, octocrab::Error> = crab
    ...
    match gh_cmd {
        cli::GitHubCommands::Issues => {
            list_issue_counts(config)
                .await
                .expect("Could not load issue counts.");
        }
    }
}
